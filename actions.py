...
def perform_action(
    ...
    **legacy_kwargs,
    ...
    # Backward-compat kwargs in case caller passes old/merged names.
    if "ui settle seconds" in legacy_kwargs:
        ui_settle_seconds = float(legacy_kwargs["ui settle seconds"])
    if "ui_settle" in legacy_kwargs:
        ui_settle_seconds = float(legacy_kwargs["ui_settle"])
    if "hit_sleep_min" in legacy_kwargs:
        hit_sleep_min_seconds = int(legacy_kwargs["hit_sleep_min"])
    if "hit_sleep_max" in legacy_kwargs:
        hit_sleep_max_seconds = int(legacy_kwargs["hit_sleep_max"])
